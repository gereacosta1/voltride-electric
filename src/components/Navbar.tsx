...
function NavButton({
  ...
  return (
    <button
      className="rounded-xl px-3 py-2 text-sm font-semibold text-white/70 transition hover:bg-white/7 hover:text-white"
    ...
    <header className="sticky top-0 z-50">
      ...
      <div className="relative mx-auto max-w-6xl px-4 pt-4">
        <div className="group flex items-center justify-between gap-3 rounded-[28px] border border-white/10 bg-[#0d1320]/75 px-4 py-3 shadow-[0_18px_70px_rgba(0,0,0,.35)] backdrop-blur-2xl transition hover:border-white/15 hover:bg-[#0d1320]/85">
          ...
          <nav className="hidden items-center gap-1 rounded-2xl border border-white/5 bg-white/[0.025] p-1 md:flex">
            <NavButton target="catalog">Catalog</NavButton>
            <NavButton target="store">Store</NavButton>
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            >
              ...
              {count > 0 ? (
                <span className="absolute -right-2 -top-2 inline-flex h-6 min-w-6 items-center justify-center rounded-full border border-white/25 bg-gradient-to-r from-fuchsia-500 to-lime-300 px-2 text-xs font-black text-black shadow-lg">
                  {count}
                </span>
              ) : null}
            </button>
